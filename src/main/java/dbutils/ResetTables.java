...
public class ResetTables {
	...
	public void resetADSRentDerbyDB() throws FileNotFoundException, IOException, SQLException, PersistenceException {
		DataSource.INSTANCE.connect("jdbc:derby:data/derby/adsdb;create=true", "RentSys", "");
		RunSQLScript.runScript(DataSource.INSTANCE.getConnection(), "data/scripts/resetTables-Rent-Derby.sql");
		DataSource.INSTANCE.close();		
	}
...
//		new ResetTables().resetADSDerbyDB();
		new ResetTables().resetADSRentDerbyDB();
	}
